...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-cscwrapper runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-cscwrapper.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_cscwrapper")
...
def lint(c):
    ...
    """
    c.run("flake8 django-cscwrapper tests")
...
def release(c, bumpsize=''):
    ...
    import django_cscwrapper
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_cscwrapper.__version__))
    c.run("git push --tags")
